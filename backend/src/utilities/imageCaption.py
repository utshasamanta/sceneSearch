...
def getVideoContext(videoPath):
    ...
    framesToGet = [frameBreak * i for i in range(1,11)]
    ...
    while True:
        ...
        if j < len(framesToGet) and i == framesToGet[j]:
            ...
            selectedFrames.append(pilImage)
            j += 1
        i+= 1
...
def getPhotoContext(photoPath):
    raw = Image.open(photoPath).convert('RGB')
